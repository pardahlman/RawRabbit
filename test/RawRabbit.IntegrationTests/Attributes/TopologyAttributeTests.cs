	...
	public class TopologyAttributeTests : IntegrationTestBase
	{
		...
		public async Task Should_Honor_Custom_Config()
		{
			...
			client.SubscribeAsync<AttributedMessage>((message, context) =>
			{
				tcs.TrySetResult(message);
				return Task.FromResult(true);
			}, c => c.WithRoutingKey("special").WithExchange(e => e.WithName("special")));

			/* Test */
			await client.PublishAsync(new AttributedMessage(), configuration: c => c.WithRoutingKey("special").WithExchange(e => e.WithName("special")));
			await tcs.Task;
